...
assertEquals("linker-lib-own-version", getInputLinkerLibVersion().version, "0.75");
assertEquals("linker-lib-map-version", result.map.inputLinker_lib.version, "0.75");
...
assertTrue("text-result-has-linker", textResult.indexOf("inputLinker_lib v0.75") !== -1);
...
assertEquals("getter-only-old-optional-mismatch", getterOnlyOldOptional.versionMismatch[0], "inputLinker_lib expected 0.75 got 0.39");
...
assertTrue("all-versions-text-has-linker", allVersionsText.indexOf("inputLinker_lib v0.75") !== -1);

checkAtagLibVersions({ checkAccess: true, verbose: true });
assertTrue("verbose-log-written", _logs.join("\n").indexOf("inputLinker_lib v0.75") !== -1);
