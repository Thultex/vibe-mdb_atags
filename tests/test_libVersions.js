...
assertTrue("autorun-plugin-mismatch-summary", _logs.join("\n").indexOf("System v2.50 (config/match/miss, 1 local, 15 missing - no config!)") >= 0);
...
assertEquals("checkVersions-own-version", getCheckVersionsVersion().version, "1.65");
...
assertEquals("collect-lib-own-version", getCollectAtagsLibVersion().version, "1.69");
assertEquals("export-lib-own-version", getExportAtagsLibVersion().version, "1.84");
assertEquals("tag-cleaner-own-version", getTagCleanerVersion().version, "1.55");
...
assertEquals("dust-merger-own-version", getDustMergerVersion().version, "0.17");
assertEquals("dust-merger-registered-version", ATAG_LIB_VERSIONS.dustMerger.version, "0.17");
assertEquals("template-transfer-own-version", getTemplateFieldTransferVersion().version, "1.03");
assertEquals("template-transfer-registered-version", ATAG_LIB_VERSIONS.templateFieldTransfer.version, "1.03");
...
assertEquals("collect-lib-map-version", result.map.collectAtags_lib.version, "1.69");

var dustGetterResult = checkAtagLibVersions({ names: ["dustMerger"], checkAccess: true, requireAll: false, asText: false });
assertEquals("generic-getter-dust-merger-version", dustGetterResult.map.dustMerger.version, "0.17");
...
assertEquals("generic-getter-template-transfer-version", templateTransferGetterResult.map.templateFieldTransfer.version, "1.03");
...
assertEquals("registered-optional-tag-cleaner", nonLib.map.tagCleaner.version, "1.55");
...
assertTrue("text-result-starts-with-summary", textResult.indexOf("System v2.50 (ok, 3 rm, 17 local)") === 0);
assertTrue("text-result-has-export", textResult.indexOf("exportAtags_lib v1.84") !== -1);
assertTrue("text-result-has-collect", textResult.indexOf("collectAtags_lib v1.69") !== -1);
assertTrue("text-result-has-local-tag-cleaner", textResult.indexOf("LOCAL tagCleaner v1.55") !== -1);
assertTrue("text-result-has-local-dust-merger", textResult.indexOf("LOCAL dustMerger v0.17") !== -1);
assertTrue("text-result-has-local-template-transfer", textResult.indexOf("LOCAL templateFieldTransfer v1.03") !== -1);
...
assertTrue("hidden-local-summary-keeps-count", hiddenLocalText.indexOf("System v2.50 (ok, 3 rm, 17 local)") === 0);
assertTrue("hidden-local-lines-hidden", hiddenLocalText.indexOf("LOCAL tagCleaner v1.55") < 0);

var hiddenUppercaseLocalText = checkAtagLibVersions({ checkAccess: true, asText: true, SHOW_LOCAL_VERSIONS: false });
assertTrue("hidden-uppercase-local-lines-hidden", hiddenUppercaseLocalText.indexOf("LOCAL tagCleaner v1.55") < 0);
...
assertTrue("scoped-config-omits-remote", scopedConfigText.indexOf("collectAtags_lib v1.69") < 0);
assertTrue("scoped-config-omits-local", scopedConfigText.indexOf("LOCAL tagCleaner v1.55") < 0);
...
assertTrue("hidden-local-missing-summary-ok", hiddenLocalMissingText.indexOf("System v2.50 (ok, 3 rm, 16 local)") === 0);
...
assertTrue("hidden-remote-missing-summary-ok", hiddenRemoteMissing.indexOf("System v2.50 (ok, 1 rm, 17 local)") === 0);
...
assertEquals("getter-only-old-mismatch", getterOnlyOldOptional.versionMismatch[0], "collectAtags_lib expected 1.69 got 1.39");
...
assertEquals("registry-only-old-mismatch", registryOnlyOldOptional.versionMismatch[0], "collectAtags_lib expected 1.69 got 1.39");
ATAG_LIB_VERSIONS = {};
...
assertEquals("registry-only-major-mismatch", registryOnlyMajorOptional.versionMismatch[0], "collectAtags_lib expected major 1 got major 2 (1.69 vs 2.00)");
ATAG_LIB_VERSIONS = savedRegistry;
...
assertTrue("hidden-remote-missmatch-summary-ok", hiddenRemoteMissmatchText.indexOf("System v2.50 (ok, 1 rm, 17 local)") === 0);
assertTrue("hidden-remote-missmatch-line-hidden", hiddenRemoteMissmatchText.indexOf("VERSION RM: collectAtags_lib expected 1.69 got 1.39") < 0);
getCollectAtagsLibVersion = savedGetCollectAtagsLibVersion;
ATAG_LIB_VERSIONS = savedRegistry;
...
assertTrue("hidden-local-missmatch-summary-ok", hiddenLocalMissmatchText.indexOf("System v2.50 (ok, 3 rm, 17 local)") === 0);
assertTrue("hidden-local-missmatch-line-hidden", hiddenLocalMissmatchText.indexOf("VERSION LOCAL: tagCleaner expected 1.55 got 1.50") < 0);
getTagCleanerVersion = savedGetTagCleanerVersion;
ATAG_LIB_VERSIONS = savedRegistry;
...
var mixedVersionRemIndex = mixedOrderText.indexOf("VERSION RM: collectAtags_lib expected 1.69 got 1.39");
var mixedMissingRemIndex = mixedOrderText.indexOf("MISSING RM: #3 Helpers Lib (core_lib)");
var mixedVersionLocalIndex = mixedOrderText.indexOf("VERSION LOCAL: tagCleaner expected 1.55 got 1.50");
...
assertEquals("optional-plugin-mismatch", optionalPluginMismatch.versionMismatch[0], "tagCleaner expected 1.55 got 1.50");
...
assertTrue("optional-plugin-mismatch-text-visible", optionalPluginMismatchText.indexOf("VERSION LOCAL: tagCleaner expected 1.55 got 1.50") !== -1);
...
assertTrue("optional-dust-merger-mismatch-register-log-visible", _logs.join("\n").indexOf("VERSION LOCAL: dustMerger expected 0.17 got 0.12") >= 0);
ATAG_LIB_VERSIONS = {};
registerAtagLibVersion("tagCleaner", "1.55", "2.50", "core/tagCleaner.js");
...
assertTrue("all-versions-text-has-collect", allVersionsText.indexOf("collectAtags_lib v1.69") !== -1);

checkAtagLibVersions({ checkAccess: true, verbose: true });
assertTrue("verbose-log-written", _logs.join("\n").indexOf("collectAtags_lib v1.69") !== -1);
