...
assertEquals("collect-lib-own-version", getCollectAtagsLibVersion().version, "1.64");
assertEquals("export-lib-own-version", getExportAtagsLibVersion().version, "1.84");
assertEquals("tag-cleaner-own-version", getTagCleanerVersion().version, "1.50");
...
assertEquals("collect-lib-map-version", result.map.collectAtags_lib.version, "1.64");
...
assertTrue("text-result-has-collect", textResult.indexOf("collectAtags_lib v1.64") !== -1);
...
assertEquals("getter-only-old-mismatch", getterOnlyOldOptional.versionMismatch[0], "collectAtags_lib expected 1.64 got 1.39");
...
assertEquals("registry-only-old-mismatch", registryOnlyOldOptional.versionMismatch[0], "collectAtags_lib expected 1.64 got 1.39");
ATAG_LIB_VERSIONS = {};
...
assertEquals("registry-only-major-mismatch", registryOnlyMajorOptional.versionMismatch[0], "collectAtags_lib expected major 1 got major 2 (1.64 vs 2.00)");
ATAG_LIB_VERSIONS = savedRegistry;
...
assertTrue("all-versions-text-has-collect", allVersionsText.indexOf("collectAtags_lib v1.64") !== -1);

checkAtagLibVersions({ checkAccess: true, verbose: true });
assertTrue("verbose-log-written", _logs.join("\n").indexOf("collectAtags_lib v1.64") !== -1);
