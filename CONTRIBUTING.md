...
Dieses Repo kann öffentlich geteilt werden. Beispiele in Code, Tests, README und Changelog sollen deshalb keine privaten Namen, echten Vault-/Bibliotheksnamen, persönlichen Gesundheitsdaten, Alltagsnotizen, Pfade, Kontakte oder realen Inhalte enthalten.

Regeln:

- Für Vaults und externe Systeme neutrale Namen wie `ExampleVault`, `ExampleLibrary` oder `ExampleField` verwenden.
- Für Tags und Kategorien neutrale Parser-Beispiele wie `SymptomA`, `SymptomB`, `Body`, `BodySafe`, `MetricA`, `TaskA` oder `ActivityA` verwenden.
- Keine echten Obsidian-Vault-Namen, lokalen Benutzerpfade, Telefonnummern, E-Mail-Adressen, Tokens oder persönlichen Notizen committen.
- Wenn ein Test reale Semantik braucht, die Semantik anonymisieren und nur die technische Struktur erhalten.
- Vor größeren Doku- oder Teständerungen kurz mit `rg` nach privaten Begriffen suchen.
...
- Jede Moduldatei hat genau einen Kopfblock; ein zweiter Kopier-Header entfällt.
- Die Systemversion bleibt unverändert, solange sich nur das einzelne Modul ändert.
- Die Kennung vor dem Namen ist Pflicht: `A` für Core, `B` für Add-ons, `C` für gelöste eigenständige Module. Die Nummer folgt der dokumentierten Repo-Reihenfolge.
...
## Verbindliche Example-Regeln

Die zentralen Example-Dateien sind Teil der öffentlichen Dokumentation. Eine Codeänderung an einer öffentlichen Funktion ist erst vollständig, wenn das zugehörige Example im selben Change aktualisiert wurde.

Als öffentliche Funktionen gelten Trigger- und Script-Funktionen, die Benutzer direkt aufrufen. Rein interne Hilfsfunktionen und Versions-Getter müssen nicht als eigene Funktion dokumentiert werden.

Für jede Example-Datei gilt verbindlich:

- Core- und Remote-Lib-Beispiele stehen in `examples.md`, Add-on-Beispiele in `examples_plugins.md`.
- Oben steht eine `## Übersicht`, die innerhalb derselben Datei auf jede dokumentierte öffentliche Funktion verlinkt.
- Jedes Modul erhält einen eigenen `###`-Abschnitt in der Reihenfolge seiner Modulkennung, zum Beispiel `### B3 globalFieldSync.js`.
- Direkt nach der Modulüberschrift steht als Blockquote ein Satz, der die übergeordnete Aufgabe des Moduls beschreibt.
- Jede öffentliche Funktion erhält einen expliziten HTML-Anker und eine deutlich beschriftete Unterüberschrift nach dem Muster `#### Funktion: syncFieldTo()`.
- Direkt nach der Funktionsüberschrift steht als Blockquote ein kurzer Satz, der zuerst erklärt, was die Funktion macht. Ein zusätzliches Label wie „Kurzbeschreibung“ wird nicht verwendet.
- Danach folgt mindestens ein vollständiger, kopierbarer Aufruf der Funktion.
- Unter `Parameter:` werden alle im Aufruf verwendeten Parameter kurz erklärt. Positionsparameter, Rückgabewert und relevante Aliase werden erklärt, wenn sie für die direkte Nutzung wichtig sind.
- Jede unterstützte neue Konfigurationsoption erscheint mindestens einmal in einem passenden, kopierbaren Example. Das gilt auch für geänderte Defaults, Modi und öffentlich zugesicherte Aliase.
- Parameter mit einer festen Wertemenge führen alle unterstützten Werte auf. Kleine Wertemengen stehen vollständig und übersichtlich direkt in der Parameterbeschreibung.
- Umfangreiche oder von mehreren Funktionen gemeinsam verwendete Wertemengen stehen einmal zentral in einer Markdown-Tabelle mit Wert, Aliasen und Wirkung. Die betroffenen Parameterbeschreibungen verlinken auf diese Tabelle. Die Aggregationsmodi für `mode`, `valueMode`, Row- und Kategorie-Aggregationen sind das verbindliche Muster dafür.
- Ändern sich Signatur, Verhalten, Standardwert oder Konfigurationsoption einer öffentlichen Funktion, werden Übersicht, Zweckbeschreibung, Aufruf und Parameterbeschreibung im selben Change geprüft und bei Bedarf angepasst.
- Neue öffentliche Funktionen werden im passenden Modulabschnitt sowie in der Übersicht ergänzt.
- Deutsche Prosa verwendet echte Umlaute und `ß`; Ersatzschreibweisen mit `ae`, `oe` oder `ue` sind nicht zulässig. Technische Bezeichner, Feldnamen und fremdsprachige Inhalte bleiben unverändert.
- Beispiele enthalten keine privaten Daten; die Regeln aus „Datenschutz in Beispielen“ gelten unverändert.
- Ausführliche Verhaltens- und Versionsänderungen gehören in `CHANGELOG.md`; die Example-Dateien bleiben auf Anwendung und Parameter konzentriert.

Vor Abschluss einer passenden Codeänderung muss `tests/test_exampleDocs.js` erfolgreich laufen. Der Test prüft Übersicht, Anker, Überschriftenhierarchie, Zweck-Blockquotes, Aufrufe, Parameterabschnitte, Modulbeschreibungen, zentrale Optionswert-Tabellen, relevante Konfigurationsoptionen und deutsche Umlautschreibweisen.
...
- `core/_checkVersions.js` ist der Checker/Loader, keine Remote-Lib; `checkAtagLibVersions({ checkAccess: true })` prüft die erwarteten Remote-Libs und ihre aufrufbaren Versionsfunktionen.
- `core/helpers.js` gehört funktionell zur Lib-Nutzung, bleibt aber Memento-spezifisch, verweist auf `core_lib/helpers_lib.js` und wird nicht in `checkLibVersions()` registriert.
...
- Wenn eine Remote-Lib-Version steigt, `core/_checkVersions.js`, `core_lib/Z_LIB_VERSIONS.md` und die Versionstests mitprüfen.
...
Tests und Verifikation sollen zur Größe und zum Risiko der Änderung passen.

Regeln:

- Besonders bei Add-ons vorab prüfen, ob ein eigener Test wirklich wichtig ist.
- Eigene Add-on-Tests sind vor allem sinnvoll bei Parser-/Regex-Logik, Idempotenz, Datenverlust-Risiko, Feld-Schreiblogik, Memento/WSH-Kompatibilität oder mehreren relevanten Verzweigungen.
...
- Nicht für jede kleine oder klar lokale Änderung automatisch eine neue Testdatei anlegen.
...
- Kleine Änderungen dürfen auch nur mit Syntax-Check, kurzer Laufzeitprüfung oder gezielter manueller Verifikation abgeschlossen werden.
...
- Bei Änderungen an öffentlicher API, Verhalten, Defaults oder Konfigurationsoptionen den zugehörigen Abschnitt in `examples.md` oder `examples_plugins.md` nach den verbindlichen Example-Regeln angepasst
- Bei Änderungen an öffentlichen Funktionen `cscript //Nologo tests\\test_exampleDocs.js` erfolgreich ausgeführt
