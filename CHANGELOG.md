...
- Change: `Input Linker` v0.87 gibt `PostEntry` optional ein zweites Argument aus `postEntryOptions`/`postEntryArgs` weiter; damit können riskante Save-/Load-Schritte wie `restoreAtags()` außerhalb des normalen Linker-Durchlaufs explizit freigeschaltet werden. v0.85 entfernte die kritischen Restore-/EnsureActive- und DayId-Schreibfunktionen.
- Feature: `Input Linker` v0.84 warnt bei Receive/Refresh standardmäßig im Ziel-Debug und oben in der Ziel-Notiz mit `ACHTUNG: Datei im Papierkorb!`, wenn der Ziel-Eintrag als gelöscht/Papierkorb erkennbar ist; abschaltbar mit `checkTargetTrash: false`.
...
- *Versionen: sys 2.40, collectAtags_lib v1.61, Input Linker v0.87, libVersions v1.28, Time Marker v1.37, Tag Pair Parser v1.01, restoreAtags v2.08, Global Field Sync v1.03, Sync Last From Latest v1.05, Sequence Counter v1.05.*
