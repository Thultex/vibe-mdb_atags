...
- Change: Check Versions v1.68 erwartet `collectAtags_lib` v1.72.
...
- Change: Template Field Transfer v1.03 lässt `_00` als normales `tag00` für die zentrale Cleaner-Regel stehen.
- Change: Check Versions v1.63 erwartet Tag Cleaner v1.55 und Template Field Transfer v1.03.
...
- Change: `Input Linker` v0.88 führt kein `recalcTarget` mehr aus, macht die Papierkorb-Warnung opt-in über `checkTargetTrash: true` und nutzt `DayId` wieder als lesenden Refresh-Check ohne in Inputs zu schreiben; v0.87 gab `PostEntry` optional ein zweites Argument aus `postEntryOptions`/`postEntryArgs` weiter.
- Feature: `Input Linker` v0.84 ergänzte die Papierkorb-Warnung; seit v0.88 ist sie nicht mehr standardmäßig aktiv.
- Fix: `Input Linker` v0.83 bringt den Neu-Link-Pfad näher an die stabile Link-only-Version: `linkInputEntryToTarget()` schreibt keine `DayId` mehr in den Input und ruft keinen Restore-/EnsureActive-Nachlauf mehr auf.
- Fix: `Input Linker` v0.82 entfernt den zweiten `refreshBeforeOpen`-Receive aus dem Link-Pfad; neue Links schreiben höchstens einmal über `receiveAfterLink`, bestehende Links mit `receiveExistingLink: false` bleiben ein echter No-op ohne Debug-Schreibzugriff.
...
- *Versionen: sys 2.40, collectAtags_lib v1.61, Input Linker v0.88, libVersions v1.29, Time Marker v1.37, Tag Pair Parser v1.01, restoreAtags v2.08, Global Field Sync v1.03, Sync Last From Latest v1.05, Sequence Counter v1.05.*
